...
class TransformerEncoderLayer(BaseModule):
    ...
    def forward(self, x):
        h=x
        x,weight=self.attn(self.ln1(x))
        x = h + x
        x = self.ffn(self.ln2(x), identity=x)
        # return x,weight  
        return (x + x.mean(dim=1, keepdim=True)) * 0.5,weight


@MODELS.register_module()
class TransFG(BaseBackbone):
    """Vision Transformer.
    ...
    def __init__(self,
                 ...
                 layer_scale_init_value=0.,
                 patch_cfg=dict(), #切割patch是否重叠，non-overlap 不重叠 overlap 重叠 为overlap时 要设置 patch_cfg={stride=?}   padding=0 和论文保持一致
                 layer_cfgs=dict(),
                 pre_norm=False,
                 init_cfg=None):
        super(TransFG, self).__init__(init_cfg)
        ...
        for i in range(self.num_layers - 1):  # 替换最后一层
            _layer_cfg = dict(
                embed_dims=self.embed_dims,
        ...
        _part_layer_cfg = dict(
                ...
                qkv_bias=qkv_bias,
                norm_cfg=norm_cfg)
        _part_layer_cfg.update(layer_cfgs[-1])
        self.part_layer = TransformerEncoderLayer(**_part_layer_cfg)
        self.part_norm = build_norm_layer(norm_cfg,self.embed_dims)
    ...
    def init_weights(self):
        super(TransFG, self).init_weights()
    ...
    def forward(self, x):
        B = x.shape[0]
        x, patch_resolution = self.patch_embed(x) # (B, out_h * out_w, embed_dims), (out_h,out_w)

        if self.cls_token is not None:
            # stole cls_tokens impl from Phil Wang, thanks
            cls_token = self.cls_token.expand(B, -1, -1) #B, 1, embed_dims
            x = torch.cat((cls_token, x), dim=1) # B, 1+out_h * out_w, embed_dims

        x = x + resize_pos_embed(
            self.pos_embed, #1, 1+out_h * out_w, embed_dims
            self.patch_resolution, # (out_h,out_w)
            patch_resolution, # (out_h,out_w)
        ...
        return self.part_norm(part_states)
...
def scaled_dot_product_attention_pyimpl(query,
    ...
    return attn_weight @ value, weight
...
class MultiheadAttention(BaseModule):
    ...
    def forward(self, x):
        ...
        x, weight = self.scaled_dot_product_attention(q, k, v, dropout_p=attn_drop)
